...
```bash
lattice doctor              # 全面健康检查（只读，不修改）
lattice doctor --fix        # 检查 + 自动修复安全项
lattice doctor --migrate    # legacy 字段迁移 + DB 回填（等价于 --fix 的子集 + 字段升级）
```
...
| 重复项目检测 | 同一 localPath 被多个项目 ID 引用 | 重复 `lattice link`、手动创建 lattice.json | `lattice project remove <多余ID> -f` |
...
| 项目索引 | localPaths 指向的目录不存在 | 项目文件夹被移动/删除 | `lattice scan` 重新扫描，或 `project list --orphaned` 查看并清理 |
| RAG 索引 | 向量/FTS 索引状态异常 | 首次初始化未 rebuild、spec 大量变更后未更新 | `lattice rag rebuild` |
| FTS 索引版本 | 全文索引版本过旧（中文检索失效） | 系统升级但未重建索引 | `lattice rag rebuild` |

## 典型问题场景

### 场景 1：`project list` 项目数量异常（缺失项目）

**症状**：`lattice project list` 显示的项目数远少于预期。
...
**根因**：
- 在同一目录多次执行 `lattice link`（每次生成不同 ID）
...
2. 移除多余的：`lattice project remove <多余ID> -f`
3. 如有必要，修改 `lattice.json` 中的 id 指向保留的 ID

### 场景 3：lattice.json 与注册 ID 不匹配

**症状**：`lattice context` 或 `lattice link` 识别的项目与预期不同。
...
lattice project list --json | grep <id>
```

**修复**：
- 编辑 `<project-dir>/lattice.json` 将 id 改为正确值
- 或 `lattice unlink -f && lattice link`（重新注册会生成新 ID）

### 场景 4：搜索无结果或结果明显不全

**症状**：`lattice search` 找不到已知存在的 spec/任务。

**诊断**：
```bash
lattice rag status     # 查看索引文档数
lattice doctor         # 看 RAG 索引和 FTS 版本
```

**修复**：
```bash
lattice rag rebuild    # 全量重建索引
```
...
lattice doctor --fix   # 重建 DB 并从磁盘回填所有项目
lattice rag rebuild    # 重建搜索索引
```
