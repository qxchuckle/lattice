...
> **本文权威范围**：项目身份多 ID 模型 / 进入未知目录的 `project where` / `ltc link`（含 ID 匹配、嵌套自动检测、`--restore` / `--force-new`）/ lattice.json 指向失效的修复 / 任务的 `projects[]` 与 `scopePaths[]` / 项目关系（含 AI 推断）/ `ltc doctor` 数据完整性维护 / `project merge` 物理合并 / 自动扫描与定时扫描。
>
...
> 何时读：需要批量发现和注册本地 git 项目时 → 下一步：`ltc init scan` 或等待定时扫描自动触发。
...
- CLI 启动时自动检查 `~/.lattice/cache/last-scan.json`，距上次扫描超过 12h 则自动触发
