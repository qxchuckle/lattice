...
> **本文权威范围**：项目身份指纹模型 / 进入未知目录的 `project where` / `lattice link`（含相似检测、嵌套自动检测、`--restore` / `--force-new`）/ lattice.json 指向失效的修复 / 任务的 `projects[]` 与 `scopePaths[]` / 项目关系（含 AI 推断）/ `lattice doctor` 数据完整性维护。
>
> 进入项目后的默认 `lattice context` 上下文铺底 / 跨项目相似需求搜索见 [project-context.md](project-context.md)；终端输出过滤规则见 [SKILL.md#终端输出读取原则](SKILL.md#终端输出读取原则)。
>
...
> 何时读：进入一个不确定是否已注册到 Lattice 的目录、或 `lattice context` 报"未在 Lattice 中找到项目"时 → 下一步：根据输出三层判定走 `lattice link` 或 `--restore` / `--force-new`。
...
> 何时读：`lattice context` / `lattice status` 报"项目 id 不存在"或类似异常时 → 下一步：从下面 3 个修复路径里挑一个。
...
> 何时读：任务涉及未注册的额外仓库 / 共享组件 / 数据样本路径，或需要把任务关联 / 解绑到项目时 → 下一步：用 `lattice task associate` 调整 `projects[]` 或 `scopePaths[]`。本节也是 [task-workflows.md#项目关联同步](task-workflows.md#项目关联同步) 的权威实现。
...
> 何时读：[project-context.md#项目关系](project-context.md#项目关系) 提示存在多项目 / 跨仓库依赖、或 AI 在 PRD / 指纹里看到跨项目证据时 → 下一步：先列现有关系，确认无重复后再用 `--ai-inferred` 添加。
...
> 何时读：`lattice doctor` 报警 / 旧版数据迁移 / `project list --orphaned` 大量孤儿项目时 → 下一步：按问题类型选 `--migrate` / `--rebuild-fingerprints` / `--recheck-scope-paths`。
