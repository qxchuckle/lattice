...
扫描本地 git 项目并注册到 Lattice（可独立使用，也可在 `ltc init` 主流程末尾交互式触发）。**写入扫描缓存**，定时扫描机制依赖此缓存。

- `-f, --force`：跳过确认
- `--dirs <dirs>`：扫描目录，逗号分隔；不传则使用配置中的 `scanDirs`，都无则交互式询问并写入配置
- `--auto`：使用配置中的 `scanDirs`，跳过交互

> **与 `ltc scan` 的区别**：`init scan` 写扫描缓存（定时扫描 12h 判断依赖）、有初始化检查、支持交互式配置输入、有确认步骤。常规扫描推荐使用本命令。

> **定时扫描**：CLI 启动时自动检查 `~/.lattice/cache/last-scan.json`，距上次扫描超过 12h 则自动触发。只在扫描成功后更新时间。
...
> **与 `ltc init scan` 的区别**：`ltc scan` 是早期简单实现，不写扫描缓存（定时扫描不依赖它）、不支持交互式配置、无确认步骤。常规扫描推荐使用 `ltc init scan`。
