...
- `--query <text>`：语义化查询（主题/意图/任务描述）——补充搜索相关的 spec、任务、项目，作为「语义关联」节追加到上下文输出（已去重：不会重复显示 context 中已列出的 spec）
- `--current-user`：仅显示当前用户数据
- `--json`：以 JSON 输出

> **AI 调用时必须带 `--query`**：传入当前任务主题、用户意图或任务描述，让上下文输出自动携带语义关联的 spec、参考任务和项目，减少额外 search 调用。示例：`ltc context --query "搜索功能增强"`。
