...
2. 即将执行的 ltc 命令属于「读类」（不会改写 db / 文件 / 任务状态）
...
```bash
ltc context             # 项目级 / 用户级 / 全局级 spec 全量
ltc status              # 全局活跃任务与索引状态
ltc task list --current # 当前未完成任务
ltc spec list           # 分层 spec 清单
```
...
ltc search "<关键词A>" --type relation --json
```

**subagent 回报格式**：top-K（≤ 5）的 `{path, score, type, title}` + 一句话相关性判断；主线仅基于此决定是否再 `ltc spec show` / 读 PRD。
...
```bash
# 对 N 个 task id 并行
ltc context --task <id1>
ltc task progress <id2> --last 10
ltc spec show <task-prd>
```
...
```bash
ltc project info <候选1>
ltc project info <候选2>
...
- 健康巡检可只跑 `ltc doctor`（自身已聚合多项指标），其余按需补充
