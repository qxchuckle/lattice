...
**硬约束：以下场景必须委派，禁止主线直接执行对应命令组合。** 平台不支持 subagent 时退化为串行执行（按同样命令清单顺序跑，主动收缩输出）。免委派客观条件见 [subagent-delegation.md#条件委派原则]。
...
上表场景默认必须委派（不支持时退化串行）。命中下表任一客观条件时免委派，主线直接串行执行该 subagent 定义的同一命令清单（带 `--json`、主动收缩输出）：

| 场景 | 免委派客观条件 | 免委派时主线额外义务 |
|---|---|---|
| 信息收集类（起手 / 铺底 / 规范全文读取 / 跨项目搜索 / 影响分析） | 主线本会话已 Read 该流程所需文档全文 | 直接续用已读内容 |
| 任务归档 | 本会话无压缩 / 失忆恢复信号（[lattice-rules.md#五、失忆恢复]） | 仍读磁盘 progress / PRD 核对后再写总结 |
| fast-start 模式（铺底 + 规范读取） | 当前处于 fast-start 轻量模式 | 主线串行 `ltc context` + 选读；复杂度升高转正常模式后恢复委派 |

**`lattice-task-handoff` 永不免委派**：其触发即主线上下文缺失，不满足任何免委派条件。

**违规识别**：主线声称满足免委派条件，但本会话无对应 Read 记录、或存在压缩 / 失忆信号却跳过委派 → 条件不成立，必须补委派。
