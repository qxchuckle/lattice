...
**第一步**：从 context 输出的 spec 列表（每项已含路径+摘要）中，按标题+描述筛选可能相关的 spec → 直接 Read 该项路径读全文 → 判断是否确实相关，保留相关的，剔除无关的

**第二步**：context 已按主题语义检索；多次调用 `ltc search`，每次用空格隔开多个相关关键词形成关键词组（如 `ltc search "keyA keyB" --json`），用不同关键词组补漏 context 未覆盖的核心概念、同义词、模块名，直到信息充分 → 对新发现的高相关 spec 直接 Read 其返回路径+筛选；相关任务只记列表
