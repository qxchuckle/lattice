...
跨项目 AI 上下文管理硬约束。每条 `[xxx.md#章节]` 跳子文档展开，本文不复述流程。
...
5. 非 cwd 路径且不执行 ltc → `ltc project register <paths...>`（[project-discovery.md#自动注册（守卫）]）
...
1. 读取/参照了新 spec → `ltc task ref-spec <task-id> <spec-id>`
