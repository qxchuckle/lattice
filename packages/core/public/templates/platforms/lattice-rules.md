...
2. 如果输出显示有"活跃任务"：立即 `lattice task info <id>` 看 PRD + `lattice task progress <id>` 看 checkpoint 历史
...
1. **PRD 补全**：把"实施完成情况"和"任务完成总结"段落补到 PRD 末尾
2. **summary checkpoint**：`--type summary` 写最终成果
3. **rag update**：`lattice rag update` 让本次产出可被未来搜索
4. （可选）**spec 沉淀**：若本次形成长期规则，新建/更新 project/user/global spec
5. （可选）**archive**：完成且确认无后续 → `lattice task archive`

跳过 PRD 补全 / summary / rag update = 任务记忆不完整。
