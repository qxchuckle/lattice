...
> 用户也可手动触发 `/lattice/keep`（轻量、高频，适合每条消息都加）进行保持性自检：含任务身份 / 工作流约束 / spec **清单** / PRD 范围 / 漂移盘点；spec 清单丢失时由 `keep` 跳 `lattice context` 重拉。**spec 内容**认知丢失到无法判断行为合规性，或 `keep` 自检发现其他**严重漂移**，才走本节完整恢复流程。两者分工：`keep` = 高频轻量对齐 + spec 清单层校验；本节 = 重型重对齐 + spec 内容认知重建。
