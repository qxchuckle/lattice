...
1. `lattice context` —— 拿当前项目聚合上下文（→ [project-context.md#进入项目默认动作](project-context.md#进入项目默认动作)）
...
| 项目关联同步 | task.json 的 projects/scopePaths 与实际修改范围不一致 → 后续 `task list --current` / `lattice context` 误判 |
...
2. `lattice context` + 精读相关 spec + read_file PRD / design.md（→ [project-context.md#进入项目默认动作](project-context.md#进入项目默认动作)）
3. `lattice task list --current` → `task info` → `task progress`（→ [task-workflows.md#task-start-后的起手动作](task-workflows.md#task-start-后的起手动作)）
