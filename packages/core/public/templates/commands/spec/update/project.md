...
> spec 不只是约束 AI 行为的规范，也是 AI 认识当前项目的稳定经验。两类内容都适合沉淀为项目级 spec。

**行为约束类**：

- 形成了只对当前项目有效的约定（编码规范、提交流程、命名规则）
- 明确了当前项目的架构边界、目录规范、依赖方向、联调方式、部署流程
- 修复了一个该项目反复出现的问题，并总结出稳定做法

**项目认知类**：

- 沉淀当前项目的结构地图（哪个包做什么、关键模块职责、目录约定）
- 沉淀本项目独有的领域概念、业务术语、数据结构语义
- 记录关键设计动机、架构选型理由、被否决的方案与原因
- 沉淀解决某类问题的标准方法论（迁移流程、调试套路、定位手段）
- 描述模块入口、调用链概览、扩展点与适配方式
