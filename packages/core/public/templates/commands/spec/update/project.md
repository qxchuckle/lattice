# /lattice/spec/update/project

> **[执行前必读]** 执行本命令前必须先用 Skill 工具调用 `lattice` skill，再继续后续步骤。

**目标**：把当前会话中对"当前项目"长期有效的经验沉淀到项目级 spec。

## 什么时候使用

判定核心问题：**下次有人 / AI 进入这个项目，是否还需要这条信息？**

spec 同时承担行为约束 + 项目认知两种职能（详见 skill `spec-workflows.md` 的「spec 的双重职能（核心定义）」）。两类内容都适合沉淀为项目级 spec：

- **行为约束类**：只对当前项目有效的编码规范 / 提交流程 / 命名规则 / 架构边界 / 目录规范 / 依赖方向 / 联调方式 / 部署流程 / 反复出现问题的稳定做法
- **项目认知类**：本项目的结构地图（哪个包做什么、模块职责、目录约定）/ 独有的领域概念 / 业务术语 / 数据结构语义 / 关键设计动机 / 架构选型理由 / 被否决的方案与原因 / 解决某类问题的标准方法论 / 模块入口 / 调用链概览 / 扩展点与适配方式

完整沉淀判定（必须沉淀 / 建议沉淀 / 不沉淀）见 skill `spec-workflows.md` 的「沉淀判定（统一标准）」。

## 执行步骤

**统一遵循 skill `spec-workflows.md` 的「spec 写入流程（先读后写 + 二次审阅）」 的六步流程**：

1. 前置信息采集（先读后写，强制）
2. 检查模板与已有 spec
3. 冲突检测（语义层面）
4. 写入到当前项目对应的 spec 目录
5. 写入后二次审阅（强制）
6. `lattice rag update`（详见 skill `SKILL.md` 的「索引维护」）

### 项目级特有的写入原则

- **一个文件只聚焦一个主题**
- 写可执行规则，不写空泛口号
- 优先记录"什么时候这样做、什么时候不要这样做"
- 不要把一次性临时需求写成长期规范
- **项目级 spec 不需要 `## 适用范围` 段落**（天然只对当前项目生效）

### 项目级特有的层级判定

判断该规则应该停在项目级还是上提到 user / global：

- 该规则在其他类似项目里也有效 → 考虑提到用户级
- 已是多用户、多项目都成立的默认规则 → 考虑提到全局级

详见 skill `spec-workflows.md` 的「选择写入层级」。

## 输出要求

- 说明更新了哪个 spec 文件
- 说明新增 / 修改的核心规则
- 检测到语义冲突时明确告知：冲突点 / 项目级会覆盖上层规则 / 建议处理方式（保留特例 / 合并 / 删除冗余）
