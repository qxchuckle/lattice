...
**[依赖文档]**：
- task-workflows.md：任务模式（design vs implementation）/ 标题归纳与查重 / checkpoint 类型
- lattice-rules.md：回答闭合自检（#十、回答闭合自检，design 适用项：ref-spec / 项目注册 / 项目关系）
...
| ltc context / search / spec show | 修改任何业务代码文件 |
...
- `design <描述/主题>` → 走"标题归纳与查重"创建任务（[task-workflows.md#命令参数非任务 ID 时]）→ 进入讨论模式
...
按 [task-workflows.md#命令参数非任务 ID 时] 完成**项目定位（第 0 步，必做）** + 归纳标题 + 查重 + 创建任务（参数由第 0 步定位结果决定），然后：
...
4. **回答闭合**：每轮回答发出前按 [lattice-rules.md#十、回答闭合自检] 审查适用项（ref-spec / 项目注册 / 项目关系 / checkpoint）
...
```bash
ltc task checkpoint <task-id> --type decision --title "<决策标题>" -m "<决策内容>"
```
