...
用户确认后执行 [fast-start-workflows.md#转正常模式（to-normal）]。
...
不强制每轮都记——日志是可选的追溯手段（[fast-start-workflows.md#轻量日志]）。

## spec 沉淀

fast-start 模式**不创建任务**，但 spec 沉淀能力不受影响：

- 发现可复用内容（行为约束 / 项目认知 / 试错经验）→ 按 [spec-workflows.md#沉淀判定] 主动询问用户是否沉淀
- 用户可随时使用 `/lattice/spec/update/{project|user|global}` 命令沉淀
- 沉淀流程与正常模式一致（[spec-workflows.md#写入流程]）

## 归档

fast-start 模式下执行 `/lattice/task/archive` 时，因未创建任务，归档命令会先创建任务再按正常流程归档（[fast-start-workflows.md#归档]）。

## 约束

- 不创建任务、不写 PRD、不打 checkpoint（这些是 fast-start 的核心特征，不是遗漏）
- 不走 [lattice-rules.md#三、实施期循环]
- spec 全文读取仍然必做——fast-start 只是省略任务记录，不省略项目认知
