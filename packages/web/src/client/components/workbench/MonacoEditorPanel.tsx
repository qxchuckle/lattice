...
const EXT_LANG_MAP: Record<string, string> = {
  ...
  json: 'json', md: 'markdown', css: 'css', less: 'less', scss: 'scss',
  html: 'html', yaml: 'yaml', yml: 'yaml', py: 'python', rs: 'rust',
  go: 'go', sh: 'shell', bash: 'shell', sql: 'sql', toml: 'ini',
};
...
export function MonacoEditorPanel() {
  ...
  if (snap.diffView) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        <div style={{ padding: '6px 12px', background: '#1a1a2e', display: 'flex', alignItems: 'center', gap: 8, borderBottom: '1px solid #333' }}>
          <span style={{ color: '#faad14', fontWeight: 600 }}>⑃ Diff:</span>
          <span style={{ color: '#ccc', fontSize: 12 }}>{snap.diffView.path}</span>
          <div style={{ marginLeft: 'auto', display: 'flex', gap: 6 }}>
            <button onClick={handleAcceptDiff} style={btnStyle('#52c41a')}>✓ 接受</button>
            <button onClick={handleRejectDiff} style={btnStyle('#ff4d4f')}>✗ 拒绝</button>
          </div>
        </div>
        <DiffEditor
          height="100%"
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      {/* Tab 栏 */}
      <div style={{ display: 'flex', background: '#1a1a2e', borderBottom: '1px solid #333', overflow: 'auto' }}>
