      ...
      <div style={dialogStyle}>
          ...
          {nodesToDelete.map((n) => (
            <div key={n.id} style={{ fontSize: 12, color: '#888', padding: '2px 0' }}>
              ...
              {n.content[0] && 'text' in n.content[0]
                ? n.content[0].text.slice(0, 50)
                : n.id.slice(0, 12)}
            </div>
          ))}
        </div>

        <p style={{ fontSize: 12, color: '#faad14', margin: '0 0 16px' }}>⚠ 此操作不可撤销</p>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
          ...
          <button
            onClick={() => {
              const rollback =
                (document.getElementById('rollback-code') as HTMLInputElement)?.checked ?? false;
