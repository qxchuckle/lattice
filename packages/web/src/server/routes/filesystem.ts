...
export function registerFilesystemRoutes(app: FastifyInstance): void {
  ...
  app.get('/api/fs/list', async (req) => {
    ...
    if (!dirPath) return fail('bad_request', 'path required');

    const username = await getUsername();
    if (!(await isPathSafe(dirPath, username))) {
      return fail('forbidden', 'path not allowed');
    }
  ...
  app.get('/api/fs/tree', async (req) => {
    ...
    if (!roots) return fail('bad_request', 'roots required');
    ...
    return ok({ trees: result });
  });

  // 读文件
  app.get('/api/fs/read', async (req) => {
    const { path: filePath } = req.query as { path?: string };
    if (!filePath) return fail('bad_request', 'path required');
    ...
    try {
      ...
      if (s.size > MAX_FILE_SIZE) {
        return fail('bad_request', `file too large (${s.size} bytes)`);
      }
      const content = await readFile(filePath, 'utf-8');
      return ok({ content, size: s.size, name: basename(filePath), ext: extname(filePath) });
    } catch (err) {
      return fail('exec_failed', (err as Error).message);
    }
  });
  ...
  // 写文件
  app.post('/api/fs/write', async (req) => {
    const { path: filePath, content } = req.body as { path?: string; content?: string };
    if (!filePath || content === undefined) return fail('bad_request', 'path and content required');
  ...
  app.post('/api/fs/rename', async (req) => {
    ...
    if (!from || !to) return fail('bad_request', 'from and to required');
  ...
  app.post('/api/fs/delete', async (req) => {
    ...
    if (!targetPath) return fail('bad_request', 'path required');
