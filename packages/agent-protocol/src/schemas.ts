...
export const clientMessageSchema = z.discriminatedUnion('type', [
  ...
  z.object({
    ...
    sessionId: boundedId,
    messageId: boundedId,
    update: z.discriminatedUnion('action', [
      z.object({ action: z.literal('reorder'), newIndex: z.number().int().min(0) }),
      ...
      z.object({ action: z.literal('mode'), mode: z.enum(['queue', 'steer']) }),
    ]),
  }),
  z.object({ type: z.literal('queue.steer'), sessionId: boundedId, messageId: boundedId }),
  z.object({
